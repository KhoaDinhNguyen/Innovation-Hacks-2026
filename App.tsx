...
export default function App() {
  ...
  const handlePickupPackage = () => {
    ...
    setSimPackages((prev) =>
      prev.map((p) =>
        p.id === pkgId
          ? { ...p, status: "in_transit" as const, assignedDriverId: "user-driver", actualPickupTime: Date.now() }
          : p,
      ),
    );
    setSimDrivers((prev) =>
              ...
              {activeConflict && (
                <>
                  <Text style={styles.conflictText}>
                    ...
                    {simDrivers.find((d) => d.id === activeConflict.driverBId)?.name ?? "Driver B"} are both heading to
                    the same area.
                  </Text>
                  ...
                  <Text style={styles.conflictSuggestion}>
                    Transfer packages at the midpoint to reduce redundant driving.
                  </Text>
                ...
                <View style={styles.packageDetailCard}>
                  ...
                  {selectedPackage.status === "waiting" && canPickupMore ? (
                    ...
                    <View style={styles.packageDeliveryBadge}>
                      <Text style={styles.packageDeliveryBadgeText}>
                        In transit —{" "}
                        {simDrivers.find((d) => d.id === selectedPackage.assignedDriverId)?.name ?? "another driver"}
                      </Text>
                    </View>
